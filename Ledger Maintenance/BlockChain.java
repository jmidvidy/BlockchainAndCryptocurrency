...
public class BlockChain implements Serializable {
    ...
    /**
     * EECS 395: BlockChain and CryptoCurrency Block Validation Coding Assignment 2
     * 
    ...
    public boolean addBlock(Block block) {
        ...
        // block has no well formed parent
        BlockNode parent = blockChain.get(wrap(prevHash));
        if (parent == null) 
            return false;
        
        // block is not a valid block
        TxHandler h = new TxHandler(parent.getUTXOPoolCopy());
        Transaction[] transaction = block.getTransactions().toArray(new Transaction[0]);
        Transaction[] valids = h.handleTxs(transaction);
        if (valids.length != transaction.length) 
            return false;
        ...
        // block is past cut_off_age
        if (newHeight  > CUT_OFF_AGE + 1)
            return false;
        
        // else place the block and return true
        UTXOPool utxoPool = h.getUTXOPool();
        addCoinbaseToUTXOPool(block, utxoPool);
        BlockNode node = new BlockNode(block, parent, utxoPool);
